...
for pair in "agents/sec-expert.md:sonnet" "agents/cve-enricher.md:haiku" \
            ...
            "agents/php-runner.md:haiku" \
            "agents/ansible-runner.md:haiku" \
            "agents/netcfg-runner.md:haiku" \
            "agents/image-runner.md:haiku" \
            "agents/ai-tools-runner.md:haiku" \
            "agents/webapp-runner.md:haiku" \
            "agents/supply-chain-runner.md:haiku" \
            "agents/dep-diff-analyst.md:sonnet"; do
...
with open(path) as fh:
    for i, line in enumerate(fh, 1):
            ...
            if not isinstance(sk, list):
                ...
                for e in sk:
                    if not (isinstance(e, dict) and "tool" in e and "reason" in e):
                        ...
                        errs += 1
            continue
        # Allow the php status summary line emitted by php-runner.
        # Skip vocabulary: tool-missing, no-php-source.
        if "__php_status__" in obj:
            status = obj.get("__php_status__")
            if status not in {"ok", "partial", "unavailable"}:
                print(f"CONTRACT FAIL: {path}:{i} bad __php_status__ {status!r}", file=sys.stderr)
                errs += 1
            if not isinstance(obj.get("tools", []), list):
                print(f"CONTRACT FAIL: {path}:{i} __php_status__ tools must be a list", file=sys.stderr)
                errs += 1
            sk = obj.get("skipped", [])
            if not isinstance(sk, list):
                print(f"CONTRACT FAIL: {path}:{i} __php_status__ skipped must be a list", file=sys.stderr)
                errs += 1
            else:
                for e in sk:
                    if not (isinstance(e, dict) and "tool" in e and "reason" in e):
                        print(f"CONTRACT FAIL: {path}:{i} __php_status__ skipped entry must have tool+reason: {e!r}", file=sys.stderr)
                        errs += 1
            continue
            ...
            if not isinstance(sk, list):
                ...
                for e in sk:
                    if not (isinstance(e, dict) and "tool" in e and "reason" in e):
                        ...
                        errs += 1
            continue
        # Allow the supply-chain status summary line emitted by supply-chain-runner.
        # Skip vocabulary: tool-missing, no-supply-chain-source.
        if "__supply_chain_status__" in obj:
            status = obj.get("__supply_chain_status__")
            if status not in {"ok", "partial", "unavailable"}:
                print(f"CONTRACT FAIL: {path}:{i} bad __supply_chain_status__ {status!r}", file=sys.stderr)
                errs += 1
            if not isinstance(obj.get("tools", []), list):
                print(f"CONTRACT FAIL: {path}:{i} __supply_chain_status__ tools must be a list", file=sys.stderr)
                errs += 1
            sk = obj.get("skipped", [])
            if not isinstance(sk, list):
                print(f"CONTRACT FAIL: {path}:{i} __supply_chain_status__ skipped must be a list", file=sys.stderr)
                errs += 1
            else:
                for e in sk:
                    if not (isinstance(e, dict) and "tool" in e and "reason" in e):
                        print(f"CONTRACT FAIL: {path}:{i} __supply_chain_status__ skipped entry must have tool+reason: {e!r}", file=sys.stderr)
                        errs += 1
            continue
        # Allow the deep-deps status summary line emitted by dep-diff-analyst.
        # Skip vocabulary: no-prior-version, registry-unreachable.
        if "__deep_deps_status__" in obj:
            status = obj.get("__deep_deps_status__")
            if status not in {"ok", "partial", "unavailable"}:
                print(f"CONTRACT FAIL: {path}:{i} bad __deep_deps_status__ {status!r}", file=sys.stderr)
                errs += 1
            if not isinstance(obj.get("tools", []), list):
                print(f"CONTRACT FAIL: {path}:{i} __deep_deps_status__ tools must be a list", file=sys.stderr)
                errs += 1
            sk = obj.get("skipped", [])
            if not isinstance(sk, list):
                print(f"CONTRACT FAIL: {path}:{i} __deep_deps_status__ skipped must be a list", file=sys.stderr)
                errs += 1
            else:
                for e in sk:
                    if not (isinstance(e, dict) and "tool" in e and "reason" in e):
                        print(f"CONTRACT FAIL: {path}:{i} __deep_deps_status__ skipped entry must have tool+reason: {e!r}", file=sys.stderr)
                        errs += 1
            continue
        # Allow the secrets status summary line emitted by secrets-runner.
        # Skip vocabulary: tool-missing, no-git-history.
        if "__secrets_status__" in obj:
            status = obj.get("__secrets_status__")
            if status not in {"ok", "partial", "unavailable"}:
                print(f"CONTRACT FAIL: {path}:{i} bad __secrets_status__ {status!r}", file=sys.stderr)
                errs += 1
            if not isinstance(obj.get("tools", []), list):
                print(f"CONTRACT FAIL: {path}:{i} __secrets_status__ tools must be a list", file=sys.stderr)
                errs += 1
            sk = obj.get("skipped", [])
            if not isinstance(sk, list):
                print(f"CONTRACT FAIL: {path}:{i} __secrets_status__ skipped must be a list", file=sys.stderr)
                errs += 1
            else:
                for e in sk:
                    if not (isinstance(e, dict) and "tool" in e and "reason" in e):
                        print(f"CONTRACT FAIL: {path}:{i} __secrets_status__ skipped entry must have tool+reason: {e!r}", file=sys.stderr)
                        errs += 1
            continue
        ...
        if obj.get("origin") == "webext":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "rust":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "mobsfscan", "apkleaks", "android-lint", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "android":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "codesign", "spctl", "notarytool", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "ios":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "apkleaks", "android-lint", "systemd-analyze", "lintian", "checksec", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "linux":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "macos":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "apkleaks", "android-lint", "notarytool", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "k8s":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "iac":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "gh-actions":
            if "tool" not in obj:
                ...
                errs += 1
            # Origin-tag isolation
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "virt":
            if "tool" not in obj:
                ...
                errs += 1
            elif obj["tool"] not in {"hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder"}:
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "gosec", "staticcheck", "cppcheck", "flawfinder", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "c-cpp":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "gosec", "staticcheck", "hadolint", "virt-xml-validate", "kics", "phpcs", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
                print(f"CONTRACT FAIL: {path}:{i} c-cpp finding carries non-c-cpp tool {obj.get('tool')!r}", file=sys.stderr)
                errs += 1
        # Origin-aware validation: php findings must carry `tool` and `origin`.
        if obj.get("origin") == "php":
            if "tool" not in obj:
                print(f"CONTRACT FAIL: {path}:{i} php finding missing 'tool' field", file=sys.stderr)
                errs += 1
            elif obj["tool"] not in {"phpcs"}:
                print(f"CONTRACT FAIL: {path}:{i} php tool must be phpcs, got {obj['tool']!r}", file=sys.stderr)
                errs += 1
            # Origin-tag isolation: php findings must NOT carry any other lane's tool name.
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "gosec", "staticcheck", "hadolint", "virt-xml-validate", "kics", "cppcheck", "flawfinder", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
                print(f"CONTRACT FAIL: {path}:{i} php finding carries non-php tool {obj.get('tool')!r}", file=sys.stderr)
                errs += 1
        ...
        if obj.get("origin") == "go":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "shellcheck", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "python":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "shellcheck", "ansible-lint", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "ansible":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "shellcheck", "pip-audit", "ruff", "sing-box", "xray", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "netcfg":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "shellcheck", "pip-audit", "ruff", "ansible-lint", "trivy", "grype", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "image":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "shellcheck", "pip-audit", "ruff", "ansible-lint", "sing-box", "xray", "jq", "mcp-scan", "agentscan", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "ai-tools":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "shellcheck", "pip-audit", "ruff", "ansible-lint", "sing-box", "xray", "trivy", "grype", "bearer", "njsscan", "brakeman", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "webapp":
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "shellcheck", "pip-audit", "ruff", "ansible-lint", "sing-box", "xray", "trivy", "grype", "jq", "mcp-scan", "agentscan", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        ...
        if obj.get("origin") == "supply-chain":
            if "tool" not in obj:
                print(f"CONTRACT FAIL: {path}:{i} supply-chain finding missing 'tool' field", file=sys.stderr)
                errs += 1
            elif obj["tool"] not in {"guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner"}:
                print(f"CONTRACT FAIL: {path}:{i} supply-chain tool must be guarddog-{npm,pypi,go}|osv-scanner, got {obj['tool']!r}", file=sys.stderr)
                errs += 1
        ...
        if obj.get("origin") == "deep-deps":
            if "tool" not in obj:
                print(f"CONTRACT FAIL: {path}:{i} deep-deps finding missing 'tool' field", file=sys.stderr)
                errs += 1
            elif obj["tool"] != "dep-diff":
                print(f"CONTRACT FAIL: {path}:{i} deep-deps tool must be dep-diff, got {obj['tool']!r}", file=sys.stderr)
                errs += 1
            if obj.get("verdict") not in {"malicious", "suspicious"}:
            ...
            if obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "shellcheck", "pip-audit", "ruff", "ansible-lint", "sing-box", "xray", "trivy", "grype", "jq", "mcp-scan", "agentscan", "bearer", "njsscan", "brakeman", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner"}:
        ...
        if obj.get("origin") == "secrets":
            if "tool" not in obj:
                print(f"CONTRACT FAIL: {path}:{i} secrets finding missing 'tool' field", file=sys.stderr)
                errs += 1
            elif obj["tool"] not in {"gitleaks", "trufflehog"}:
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "webext" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "rust" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "android" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "ios" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "apkleaks", "android-lint", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "linux" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "macos" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "apkleaks", "android-lint", "notarytool", "systemd-analyze", "lintian", "checksec", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "windows" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "virt" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "shell" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "python" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "shellcheck", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "ansible" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "shellcheck", "pip-audit", "ruff", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "netcfg" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "shellcheck", "pip-audit", "ruff", "ansible-lint", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "image" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "shellcheck", "pip-audit", "ruff", "ansible-lint", "sing-box", "xray", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
for line in sys.stdin:
    ...
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "ai-tools" and obj.get("tool") in {"semgrep", "bandit", "gitleaks", "trufflehog","zap-baseline", "addons-linter", "web-ext", "retire", "cargo-audit", "cargo-deny", "cargo-geiger", "cargo-vet", "mobsfscan", "apkleaks", "android-lint", "codesign", "spctl", "notarytool", "pkgutil", "stapler", "systemd-analyze", "lintian", "checksec", "binskim", "osslsigncode", "sigcheck", "kube-score", "kubesec", "tfsec", "checkov", "actionlint", "zizmor", "hadolint", "virt-xml-validate", "kics", "phpcs", "cppcheck", "flawfinder", "gosec", "staticcheck", "shellcheck", "pip-audit", "ruff", "ansible-lint", "sing-box", "xray", "trivy", "grype", "guarddog", "guarddog-npm", "guarddog-pypi", "guarddog-go", "osv-scanner", "dep-diff"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
...
check skills/sec-audit/SKILL.md "\"supply-chain\"" "SKILL.md §2 inventory JSON missing supply-chain key"
check skills/sec-audit/SKILL.md "### 3.27 Supply-chain pass" "SKILL.md missing §3.27"
check skills/sec-audit/SKILL.md "supply-chain-runner" "SKILL.md §3.27 missing supply-chain-runner"
check skills/sec-audit/SKILL.md "__supply_chain_status__" "SKILL.md §3.27 missing supply-chain sentinel"

check skills/sec-audit/SKILL.md "## 4.5 Deep dependency diff" "SKILL.md missing §4.5 deep-deps pass"
check skills/sec-audit/SKILL.md "dep-diff-analyst" "SKILL.md §4.5 missing dep-diff-analyst"
check skills/sec-audit/SKILL.md "__deep_deps_status__" "SKILL.md §4.5 missing deep-deps sentinel"
check skills/sec-audit/SKILL.md "deep_deps_max" "SKILL.md §4.5 missing deep_deps_max cap reference"
check commands/sec-audit.md "--deep-deps" "commands/sec-audit.md missing --deep-deps flag"
check commands/sec-audit.md "deep_deps_max" "commands/sec-audit.md missing deep_deps_max emitted var"
echo "deep-deps-orchestrator: SKILL.md §4.5 + command flag document the deep-deps pass"
...
# --- v1.16 symmetry: foreign lanes reject the supply-chain + deep-deps tools.
# Proves the Stage 4 sweep (guarddog/osv-scanner/dep-diff added to every other
# lane's reject set) is live: a `go` finding mislabeled with one of the three
# new tools must be rejected. Mirrors the per-origin reject blocks.
for badtool in guarddog osv-scanner dep-diff; do
    bad_go_newtool="{\"id\":\"X\",\"severity\":\"HIGH\",\"cwe\":\"CWE-89\",\"title\":\"t\",\"file\":\"main.go\",\"line\":1,\"evidence\":\"e\",\"reference\":\"go-tools.md\",\"reference_url\":null,\"fix_recipe\":null,\"confidence\":\"high\",\"origin\":\"go\",\"tool\":\"$badtool\"}"
    if echo "$bad_go_newtool" | python3 -c '
import json, sys
errs = 0
for line in sys.stdin:
    line = line.strip()
...
echo "v1.16-symmetry: foreign lanes reject guarddog/osv-scanner/dep-diff (Stage 4 sweep live)"

# --- v1.21 secrets: the secrets whitelist rejects a foreign tool name.
bad_secrets='{"id":"x","severity":"HIGH","cwe":"CWE-798","title":"t","file":"a","line":1,"evidence":"e","reference":"secrets-tools.md","reference_url":null,"fix_recipe":null,"confidence":"high","origin":"secrets","tool":"semgrep"}'
if echo "$bad_secrets" | python3 -c '
import json, sys
errs = 0
for line in sys.stdin:
    line = line.strip()
    if not line: continue
    obj = json.loads(line)
    if obj.get("origin") == "secrets" and obj.get("tool") not in {"gitleaks", "trufflehog"}:
        errs += 1
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
    echo "contract-check: FAIL — secrets negative test: secrets finding with tool=semgrep was accepted" >&2
    fail=1
fi
echo "secrets negative-test: secrets finding with foreign tool (semgrep) correctly rejected"

# --- v1.21 symmetry: foreign lanes reject the two secrets tools. A `go` finding
# mislabeled gitleaks/trufflehog must be rejected by go's whitelist.
for badtool in gitleaks trufflehog; do
    bad_go_secrets="{\"id\":\"X\",\"severity\":\"HIGH\",\"cwe\":\"CWE-89\",\"title\":\"t\",\"file\":\"main.go\",\"line\":1,\"evidence\":\"e\",\"reference\":\"go-tools.md\",\"reference_url\":null,\"fix_recipe\":null,\"confidence\":\"high\",\"origin\":\"go\",\"tool\":\"$badtool\"}"
    if echo "$bad_go_secrets" | python3 -c '
import json, sys
errs = 0
for line in sys.stdin:
    line = line.strip()
...
echo "v1.21-symmetry: foreign lanes reject gitleaks/trufflehog"

# --- v1.25 symmetry: foreign lanes reject the virt lane's new kics tool. A `go`
# finding mislabeled kics must be rejected by go's whitelist.
bad_go_kics="{\"id\":\"X\",\"severity\":\"HIGH\",\"cwe\":\"CWE-250\",\"title\":\"t\",\"file\":\"main.go\",\"line\":1,\"evidence\":\"e\",\"reference\":\"go-tools.md\",\"reference_url\":null,\"fix_recipe\":null,\"confidence\":\"high\",\"origin\":\"go\",\"tool\":\"kics\"}"
if echo "$bad_go_kics" | python3 -c '
import json, sys
errs = 0
for line in sys.stdin:
    line = line.strip()
...
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
    echo "contract-check: FAIL — symmetry negative test: go finding with tool=kics was accepted" >&2
    fail=1
fi
...
# --- v1.27 symmetry: foreign lanes reject the php lane's phpcs tool. A `go`
# finding mislabeled phpcs must be rejected by go's whitelist.
bad_go_php="{\"id\":\"X\",\"severity\":\"HIGH\",\"cwe\":\"CWE-79\",\"title\":\"t\",\"file\":\"main.go\",\"line\":1,\"evidence\":\"e\",\"reference\":\"go-tools.md\",\"reference_url\":null,\"fix_recipe\":null,\"confidence\":\"high\",\"origin\":\"go\",\"tool\":\"phpcs\"}"
if echo "$bad_go_php" | python3 -c '
import json, sys
errs = 0
for line in sys.stdin:
    line = line.strip()
...
sys.exit(1 if errs else 0)
' >/dev/null 2>&1; then
    echo "contract-check: FAIL — symmetry negative test: go finding with tool=phpcs was accepted" >&2
    fail=1
fi
