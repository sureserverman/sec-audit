...
for reason in tool-missing no-containerfile no-libvirt-xml no-compose-file; do
...
echo '{"__virt_status__": "unavailable", "tools": [], "skipped": [{"tool": "hadolint", "reason": "tool-missing"}, {"tool": "virt-xml-validate", "reason": "tool-missing"}, {"tool": "kics", "reason": "tool-missing"}]}' > "$offline_out"
...
# Target-shape clean-skip: tools present, no Containerfile / no libvirt XML / no compose.
clean_skip='{"__virt_status__": "unavailable", "tools": [], "skipped": [{"tool": "hadolint", "reason": "no-containerfile"}, {"tool": "virt-xml-validate", "reason": "no-libvirt-xml"}, {"tool": "kics", "reason": "no-compose-file"}]}'
echo "$clean_skip" | jq -e '.skipped[] | select(.reason == "no-containerfile" or .reason == "no-libvirt-xml" or .reason == "no-compose-file")' >/dev/null \
