...
invocations, JSON output schemas, field mappings to sec-audit's finding
schema, and offline-degrade rules. Out of scope: SARIF output format
(sec-audit uses Semgrep JSON not SARIF), custom rule authoring, and other
...
# and emits findings into sec-audit's pipeline.
...
```bash
# Semgrep Registry configs that pair well with sec-audit reviews:
#   p/owasp-top-ten  — broad OWASP coverage (default for sec-audit)
...
### Recipe: Semgrep result → sec-audit finding

Semgrep `--json` emits `{"results": [...], "errors": [...], ...}`. Each
element of `results` maps to exactly one sec-audit finding as follows:

| Semgrep JSON field                   | sec-audit finding field | Notes                                                                      |
|--------------------------------------|--------------------------|----------------------------------------------------------------------------|
...
WARNING is middle, INFO is lowest. sec-audit's HIGH/MEDIUM/LOW tiers
match one-to-one.

Source: https://semgrep.dev/docs/semgrep-appsec-platform/json-and-sarif

### Recipe: Bandit result → sec-audit finding

Bandit `-f json` emits `{"results": [...], "metrics": {...}, ...}`. Each
element of `results` maps to exactly one sec-audit finding:

| Bandit JSON field   | sec-audit finding field | Notes                                                          |
|---------------------|--------------------------|----------------------------------------------------------------|
| `test_id`           | `id`                     | e.g. `B602`                                                    |
...
Plugin-to-CWE mapping table (sec-audit v0 — extend as new rules are
adopted):
