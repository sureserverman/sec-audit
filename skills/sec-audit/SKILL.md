---
name: sec-audit
description: Run a citation-grounded cybersecurity review of a project. Use when the user asks for a "security review", "CVE scan", "audit dependencies", "harden this service", "check for vulnerabilities", "OWASP review", or "scan for secrets".
---

# sec-audit — orchestrator skill
...
- **Default-to-cwd (v1.10.0+):** if the slash command resolved
  ...
  `/sec-audit` with no positional arg), echo the resolved path
...
- Confirm `target_path` is readable and is NOT the `sec-audit` plugin
...
(file a feature request, extend sec-audit with a new lane following
...
choice (e.g. an Opus-session invocation of `/sec-audit`) does NOT inflate
...
- The plugin-root path so it can read `skills/sec-audit/references/*.md`.
...
- **`--offline` mode mandatory.** The runner always passes
  ...
  lookups — sec-audit is source-only.
